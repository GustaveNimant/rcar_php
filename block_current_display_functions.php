<?php

require_once "array_functions.php";
require_once "common_html_functions.php";
require_once "language_translate_functions.php";
require_once "link_to_return_functions.php";
require_once "irp_functions.php";

$module = module_name_of_module_fullnameoffile (__FILE__);

$Documentation[$module]['what is it'] = "it is ...";
$Documentation[$module]['what for'] = "to ...";

entering_in_module ($module);

/* First Section Page Title */

function block_current_page_title_display_build () {
  $here = __FUNCTION__;
  entering_in_function ($here);

  $lan = $_SESSION['parameters']['language'];

  $sur_ent = irp_provide ('entry_surname', $here);
  $kin_blo = irp_provide ('entry_block_kind', $here);
  $nam_blo_cur = irp_provide ('block_current_name', $here);
  $sur_by_nam_a = irp_provide ('surname_by_name_array', $here);
  $sur_blo_cur = surname_of_name_of_surname_by_name_array ($nam_blo_cur, $sur_by_nam_a);

  $en_tit =  $kin_blo;
  $la_bub_tit = bubble_bubbled_la_text_of_en_text ($en_tit);
  $la_Tit = string_html_capitalized_of_string ($la_bub_tit);
  $la_Tit .= '<i><b> ' . $sur_blo_cur . '</b></i> ';

  $en_tit =  'for entry';
  $la_bub_tit = bubble_bubbled_la_text_of_en_text ($en_tit);
  $la_Tit .= lcfirst ($la_bub_tit);
  $la_Tit .= '<i><b> ' . $sur_ent . '</b></i>';

  $html_str  = comment_entering_of_function_name ($here);
  $html_str .= common_html_background_color_of_html ($la_Tit);
  $html_str .= comment_exiting_of_function_name ($here);

  debug_n_check ($here , '$html_str',  $html_str);
  exiting_from_function ($here);

  return $html_str;
}

# block_previous_sha1

function block_previous_sha1_title_display_build () {
  $here = __FUNCTION__;
  entering_in_function ($here);

  $lan = $_SESSION['parameters']['language'];

  $en_tit = "previous block sha1";
  $la_bub_tit = bubble_bubbled_la_text_of_en_text ($en_tit);
  $la_Tit = string_html_capitalized_of_string ($la_bub_tit);
  $la_Tit = '<b>' . $la_Tit . '</b>';

  $html_str = common_html_background_color_of_html ($la_Tit);
  
  debug_n_check ($here , '$html_str',  $html_str);
  exiting_from_function ($here);

  return $html_str;
}

function block_previous_sha1_value_display_build () {
  $here = __FUNCTION__;
  entering_in_function ($here);

  $blo_cur_sha = irp_provide ('block_previous_sha1', $here);

  $html_str  = '';
  $html_str .= $blo_cur_sha;

  debug_n_check ($here , '$html_str',  $html_str);
  exiting_from_function ($here);

  return $html_str;
}

function block_previous_sha1_display_build () {
  $here = __FUNCTION__;
  entering_in_function ($here);

  $html_str  = '';
  $html_str .= '<br><br>';
  $html_str .= irp_provide ('block_previous_sha1_title_display', $here);
  $html_str .= '<br>';
  $html_str .= irp_provide ('block_previous_sha1_value_display', $here);

  debug_n_check ($here , '$html_str',  $html_str);
  exiting_from_function ($here);

  return $html_str;
}

# links for actions on current item


/* Page block */

function block_current_display_build (){
  $here = __FUNCTION__;
  entering_in_function ($here);

  $nam_ent = irp_provide ('entry_name', $here);
  $sur_by_nam_a = irp_provide ('surname_by_name_array', $here);
  $sur_ent = surname_of_name_of_surname_by_name_array ($nam_ent, $sur_by_nam_a);

  $nam_blo_cur = get_hash_retrieve_value_of_key_of_where ('block_current_name', $here);
  debug_n_check ($here, '$nam_blo_cur', $nam_blo_cur);
  irp_store_force ('block_current_name', $nam_blo_cur, $here); 

  $html_str  = comment_entering_of_function_name ($here);
  $html_str .= irp_provide ('pervasive_html_initial_section', $here);
  $html_str .= irp_provide ('block_current_page_title_display', $here);

  $html_str .= irp_provide ('item_current_content_display', $here);
  $html_str .= irp_provide ('item_current_modify_display', $here);
  $html_str .= irp_provide ('item_current_justification_display', $here);
  $html_str .= irp_provide ('item_previous_content_display', $here); 

  $html_str .= irp_provide ('block_previous_sha1_display', $here); 
  $html_str .= irp_provide ('block_current_actions_display', $here);

  $html_str .= '<br><br>';
  $html_str .= link_to_return_of_entry_name_of_entry_surname_of_return_module_nameoffile ($nam_ent, $sur_ent, 'entry_display');

  $html_str .= irp_provide ('pervasive_html_final_section', $here);

  debug_n_check ($here , '$html_str', $html_str);
  exiting_from_function ($here);
  return $html_str;
}

exiting_from_module ($module);

?>