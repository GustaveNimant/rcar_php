...
function block_current_namenew_save_block_catalog_update_build () {
  $here = __FUNCTION__;
  entering_in_function ($here);
  ...
  if ( file_exists ($fno_cat_blo)) {
      $log_str = "Block Catalog $fno_cat_blo has been renamed";
  }
  else {
  ...
  $new_nam_blo_cur = irp_provide ('block_current_namenew_from_block_current_surnamenew', $here);
  $new_nam_blo_a = block_name_list_order_current_update_after_block_rename ($nam_ent_cur, $old_nam_blo_cur, $new_nam_blo_cur, $old_nam_blo_a);
...
/* getting DATA $get_val */
  ...
  $log_str  .= irp_provide ('block_current_namenew_save_block_catalog_update', $here);
